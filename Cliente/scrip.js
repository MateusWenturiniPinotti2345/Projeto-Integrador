...
const menuItems = {
  burgers: [
    { id: "b1", name: "Burger Clássico", price: 18.9, description: "Hambúrguer, queijo, alface, tomate e molho especial" },
    { id: "b2", name: "Burger Bacon", price: 22.9, description: "Hambúrguer, bacon, queijo, cebola caramelizada" },
    { id: "b3", name: "Burger Duplo", price: 28.9, description: "Dois hambúrgueres, queijo duplo, molho especial" },
    { id: "b4", name: "Burger Vegano", price: 19.9, description: "Hambúrguer de grão-de-bico, alface, tomate, molho vegano" },
    { id: "b5", name: "Burger Picante", price: 24.9, description: "Hambúrguer, pimenta jalapeño, queijo, molho picante" },
    { id: "b6", name: "Burger Gourmet", price: 32.9, description: "Hambúrguer artesanal, queijo brie, rúcula, geleia" }
  ],
  sides: [
    { id: "s1", name: "Batata Frita", price: 8.9, description: "Porção individual" },
    { id: "s2", name: "Onion Rings", price: 9.9, description: "Anéis de cebola empanados" },
    { id: "s3", name: "Nuggets", price: 12.9, description: "8 unidades" },
    { id: "s4", name: "Batata Doce", price: 10.9, description: "Fatias de batata doce" }
  ],
  drinks: [
    { id: "d1", name: "Coca-Cola", price: 5.9, description: "350ml" },
    { id: "d2", name: "Suco Natural", price: 7.9, description: "400ml" },
    { id: "d3", name: "Água", price: 3.9, description: "500ml" },
    { id: "d4", name: "Milkshake", price: 12.9, description: "400ml - Chocolate ou Morango" }
  ]
};
  ...
  if (window.elementSdk) {
    await window.elementSdk.init({
      ...
      onConfigChange: async (config) => {
        ...
        document.getElementById("delivery-fee-display").textContent = `R$ ${parseFloat(
          config.delivery_fee || defaultConfig.delivery_fee
        )
          .toFixed(2)
          .replace(".", ",")}`;
        updateCartSummary();
...
function renderMenu() {
  ...
  productsList.innerHTML = itemsToShow
    .map(
      (item) => `
...
function renderCartSidebar() {
  ...
  cartSummarySidebar.innerHTML = cart
    .map(
      (item) => `
    ...
    .map(
      ...
      <div class="flex justify-between items-center p-4 border border-gray-200 rounded-lg cart-item-enter">
        ...
        <div class="flex items-center space-x-3">
          <button onclick="updateQuantity('${item.uniqueId}', -1)" class="bg-gray-200 hover:bg-gray-300 w-8 h-8 rounded-full flex items-center justify-center font-bold">-</button>
          <span class="font-semibold w-8 text-center">${item.quantity}</span>
          <button onclick="updateQuantity('${item.uniqueId}', 1)" class="bg-gray-200 hover:bg-gray-300 w-8 h-8 rounded-full flex items-center justify-center font-bold">+</button>
          <button onclick="removeFromCart('${item.uniqueId}')" class="text-red-600 hover:text-red-700 ml-4">🗑️</button>
        </div>
      </div>
    `
    )
  ...
  addressesList.innerHTML = userAddresses
    .map(
      ...
      <label class="flex items-start p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
        <input type="radio" name="selected-address" value="${addr.address_details}" ${
        index === 0 || addr.is_default ? "checked" : ""
      } class="mr-3 mt-1" />
        <div class="flex-1">
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // Inicializar na seção de pedidos e status
  showSection("orders");
...
function renderKanbanBoard() {
  ...
  document.getElementById("pending-orders-count").textContent =
    counts.new + counts.preparing + counts.ready;
...
function createOrderCard(order) {
  ...
  card.className =
    "order-card bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow fade-in cursor-pointer";

  // Abrir detalhes (bloqueia clique quando é botão)
  ...
  if (status === "Cozinha") {
    return `
      <div class="flex space-x-2">
        <button onclick="updateOrderStatus('${order.id}', 'Aguardando entrega')"
                class="flex-1 bg-blue-500 hover:bg-blue-600 text-white text-sm py-2 px-3 rounded-lg font-medium transition-colors">⏳ Pronto</button>
        <button onclick="updateOrderStatus('${order.id}', 'Cancelados')"
                class="bg-red-500 hover:bg-red-600 text-white text-sm py-2 px-3 rounded-lg font-medium transition-colors">❌</button>
      </div>
    `;
  } else if (status === "Aguardando entrega") {
    return `
      <div class="flex space-x-2">
        <button onclick="updateOrderStatus('${order.id}', 'Saiu para entrega')"
                class="flex-1 bg-orange-500 hover:bg-orange-600 text-white text-sm py-2 px-3 rounded-lg font-medium transition-colors">🚚 Enviar</button>
        <button onclick="updateOrderStatus('${order.id}', 'Cancelados')"
                class="bg-red-500 hover:bg-red-600 text-white text-sm py-2 px-3 rounded-lg font-medium transition-colors">❌</button>
      </div>
    `;
    ...
    return `
      <button onclick="updateOrderStatus('${order.id}', 'Entregue')"
              class="w-full bg-green-500 hover:bg-green-600 text-white text-sm py-2 px-3 rounded-lg font-medium transition-colors">✅ Entregar</button>
    `;
  }
...
function getDetailedOrderActions(order) {
  ...
  if (status === "Cozinha") {
    actions = `
      <button onclick="updateOrderStatus('${order.id}', 'Aguardando entrega'); document.getElementById('order-details-modal').classList.add('hidden');"
              class="w-full bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg font-semibold mb-2">⏳ Marcar como Pronto</button>
      <button onclick="updateOrderStatus('${order.id}', 'Cancelados'); document.getElementById('order-details-modal').classList.add('hidden');"
              class="w-full bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-lg font-semibold">❌ Cancelar Pedido</button>
    `;
  } else if (status === "Aguardando entrega") {
    actions = `
      <button onclick="updateOrderStatus('${order.id}', 'Saiu para entrega'); document.getElementById('order-details-modal').classList.add('hidden');"
              class="w-full bg-orange-500 hover:bg-orange-600 text-white py-2 px-4 rounded-lg font-semibold mb-2">🚚 Enviar para Entrega</button>
      <button onclick="updateOrderStatus('${order.id}', 'Cancelados'); document.getElementById('order-details-modal').classList.add('hidden');"
              class="w-full bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-lg font-semibold">❌ Cancelar Pedido</button>
    `;
  } else if (status === "Saiu para entrega") {
    actions = `
      <button onclick="updateOrderStatus('${order.id}', 'Entregue'); document.getElementById('order-details-modal').classList.add('hidden');"
              class="w-full bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg font-semibold">✅ Marcar como Entregue</button>
    `;
  }
      ...
      <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition-shadow">
        ...
        <div class="flex space-x-2">
          <button onclick="editProduct('${product.id}')"
                  class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold transition-colors">✏️ Editar</button>
          <button onclick="toggleProductAvailability('${product.id}')"
                  class="${product.available ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'} text-white px-4 py-2 rounded-lg font-semibold transition-colors">
...
function renderCustomers(searchTerm = "") {
  ...
  if (filteredUsers.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="6" class="text-center py-8 text-gray-500">Nenhum cliente encontrado</td></tr>';
    return;
  }
  ...
  const csvContent =
    ...
    users
      .map((user) => {
        ...
        return `"${user.name}","${user.email}","${user.phone}",${userOrders.length},"R$ ${totalSpent
          .toFixed(2)
          .replace(".", ",")}","${lastOrder}"`;
      })
      .join("\n");
...
function updatePeakHours() {
  ...
  peakHoursContainer.innerHTML = hours
    .map((hourRange) => {
      const [start, end] = hourRange.split("-").map(Number);
      let count = 0;
      for (let h = start; h < end; h++) count += hourCounts[h] || 0;
...
async function toggleRestaurantStatus() {
  const currentStatus = window.elementSdk
    ? (window.elementSdk.config.restaurant_open !== undefined
        ? window.elementSdk.config.restaurant_open
...
function updateRestaurantStatusDisplay(isOpen) {
  ...
  if (isOpen) {
    statusBtn.className =
      "relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 bg-green-500";
    ...
    statusBtn.className =
      "relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 bg-red-500";
...
<head>
  ...
  <style>
    ...
    .order-info, .customer-info { margin-bottom: 15px; border-bottom: 1px dashed #000; padding-bottom: 10px; }
  ...
  <div class="items">
    ...
    ${items
      ...
      <div class="item">
        ...
        <div class="item-price">R$ ${item.price.toFixed(2).replace(".", ",")} cada = R$ ${(item.price * item.quantity)
          .toFixed(2)
          .replace(".", ",")}</div>
      </div>`
      )
  ...
  const sampleOrders = [
    {
      ...
      items: JSON.stringify([
        { name: "Burger Clássico", price: 18.9, quantity: 1 },
        { name: "Batata Frita", price: 8.9, quantity: 1 },
        { name: "Coca-Cola", price: 5.9, quantity: 1 },
      ]),
      created_at: new Date().toISOString(),
    },
    {
      ...
      total: 22.9,
      items: JSON.stringify([{ name: "Burger Bacon", price: 22.9, quantity: 1 }]),
    ...
    },
    {
      ...
      total: 54.7,
      items: JSON.stringify([
        { name: "Burger Clássico", price: 18.9, quantity: 2 },
        { name: "Batata Frita", price: 8.9, quantity: 1 },
        { name: "Coca-Cola", price: 5.9, quantity: 2 },
      ]),
